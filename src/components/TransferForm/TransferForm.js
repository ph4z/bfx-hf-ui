...
export default class TransferForm extends React.Component {
  ...
  constructor(props) {
    ...
    const algoTransfers = TransferForm.getAOs(currentExchange)
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const algoTransfers = TransferForm.getAOs(activeExchange)
  ...
  onChangeActiveOrderLayout(orderLabel) {
    ...
    const { currentExchange, algoTransfers } = this.state
    const orders = allOrders[currentExchange]

    let uiDef = orders.find(({ label }) => label === orderLabel)

    if (!uiDef) {
      uiDef = algoTransfers.find(({ label }) => label === orderLabel)
    }
  ...
  onSubmit(action) {
    ...
    const { submitTransfer, authToken, gaSubmitOrder } = this.props
  ...
  onSubmitAlgoTransfer() {
    const { submitAlgoTransfer, authToken, gaSubmitAO } = this.props
    const {
  ...
  onChangeExchange(option) {
    ...
    const algoTransfers = TransferForm.getAOs(exchange)
    const { orders } = this.props
    const currentOrder = orders[exchange][0] || algoTransfers[0]
  ...
  render() {
    ...
    algoOrders.forEach(({ label, id, uiIcon }) => algoOrderTypes.push({
      id,
      label,
      uiIcon,
    }))
    ...
    return (
      <Panel
        ...
        className='hfui-transferform__panel'
        moveable={moveable}
      ...
      >
        <div key='transferform-wrapper' className='hfui-transferform__wrapper'>
          {[
          ...
          {helpOpen && currentLayout && currentLayout.customHelp && (
            <div key='overlay-wrapper' className='hfui-transferform__overlay-wrapper'>
              <Scrollbars>
                <div className='hfui-transferform__help-inner'>
                  <p className='hfui-transferform__help-title'>
                  ...
                  <p className='hfui-transferform__help-content'>
          ...
          {!currentLayout && (
            <div key='order-transfer-menu' className='hfui-transferform__overlay-wrapper'>
              <Scrollbars>
                <TransferFormMenu
          ...
          {currentLayout && [
            <div className='hfui-transferform__layout-label' key='layout-label'>
              <i
                className='icon-back-arrow'
                onClick={this.onClearOrderLayout}
              />
              <div className='hfui-transferform__layout-label-inner'>
                <i className={`icon-${currentLayout.uiIcon}`} />
                <p>{currentLayout.label}</p>
              </div>
            </div>,

            <ul className='hfui-transferform__header' key='of-header'>
              <li key='item'>
                <Dropdown
                  icon='exchange-passive'
                  value={context}
                  key='dropdown-transferform'
            ...
            creationError && (
              <div className='hfui-transferform__creation-error' key='of-error'>
                <p>{creationError}</p>
              </div>
            ),
          ]}
